...
class SendEmojiService : AccessibilityService() {
    private var residGroup = "com.tencent.mm:id/obn"
    private var residUsername = "com.tencent.mm:id/brc"
    private var residContent = "com.tencent.mm:id/bkl"
    private var residEditText = "com.tencent.mm:id/bkk"
    ...
    private fun parseSendMessage(event: AccessibilityEvent) {
        ...
        // fetch group name
        var nodeInfo = rootInActiveWindow.findAccessibilityNodeInfosByViewId(residGroup)
        for (tv in nodeInfo) {
        ...
        // fetch all messages
        nodeInfo = rootInActiveWindow.findAccessibilityNodeInfosByViewId(residUsername)
        if (nodeInfo.size < 1) {
        ...
        nodeInfo = rootInActiveWindow.findAccessibilityNodeInfosByViewId(residContent)
        tv = nodeInfo.last()
        if (tv.className == TextView::class.java.name) {
            var content = tv.text.toString()
        ...
        if (send) {
            nodeInfo = rootInActiveWindow.findAccessibilityNodeInfosByViewId(residEditText)
            if (nodeInfo.size > 0) {
                for (et in nodeInfo) {
